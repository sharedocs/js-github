...
module.exports = function(repo, root, accessToken, githubHostname) {
    ...
    function createTree(entries, callback): any {
    ...
    function fastUpdateTree(entries, callback) {
        let request: any = { tree: entries.map(mapTreeEntry) };
    ...
    function listRefs(filter: string, callback) {
...
function encodeBlob(blob) {
    ...
    else if (bodec.isBinary(blob)) return {
...
function decodeCommit(result: any) {
    return {
